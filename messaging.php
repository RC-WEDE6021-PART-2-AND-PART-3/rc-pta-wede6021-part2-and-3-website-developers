...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="grid md:grid-cols-3 gap-8 bg-white rounded-[3rem] border border-gray-100 overflow-hidden shadow-xl shadow-gray-200/50 min-h-[700px]">
        <!-- Inbox List -->
        <div class="border-r border-gray-50 flex flex-col h-full bg-gray-50/50">
            ...
            <div class="flex-1 overflow-y-auto">
                <?php if($convResult && $convResult->num_rows > 0): ?>
                    <?php while($conv = $convResult->fetch_assoc()): ?>
                        <a href="messaging.php?sellerId=<?php echo $conv['otherUserId']; ?>" class="block p-6 hover:bg-white transition-colors <?php echo ($sellerId == $conv['otherUserId']) ? 'bg-white' : ''; ?>">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="font-bold text-gray-900"><?php echo $conv['otherUserName']; ?></div>
                            </div>
                        </a>
                    <?php endwhile; ?>
                <?php else: ?>
                    <div class="p-8 text-center text-gray-400 font-medium">No messages yet.</div>
                <?php endif; ?>
            </div>
        </div>
            ...
            ?>
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="font-bold text-lg"><?php echo $otherUser['fullName']; ?></div>
                    </div>
                </div>
                    ...
                    <?php if ($messages && $messages->num_rows > 0): ?>
                        ...
                        ?>
                            <div class="flex <?php echo $isMine ? 'justify-end' : 'justify-start'; ?>">
                                <div class="max-w-[80%] <?php echo $isMine ? 'bg-indigo-600 text-white rounded-t-2xl rounded-l-2xl' : 'bg-gray-100 text-gray-900 rounded-t-2xl rounded-r-2xl'; ?> p-4 text-sm font-medium shadow-sm">
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <div class="flex-1 flex items-center justify-center text-gray-400 italic">Start the conversation...</div>
                    <?php endif; ?>
                </div>
                ...
                <div class="p-6 border-t border-gray-100 bg-gray-50/30">
                    <form method="POST" class="flex gap-4">
                        ...
                        <input type="text" name="content" required placeholder="Type your message..." class="flex-1 bg-white border border-gray-100 px-6 py-4 rounded-full text-sm focus:ring-2 focus:ring-indigo-100 transition-all outline-none">
                        <button type="submit" name="sendMessage" class="bg-indigo-600 text-white p-4 rounded-full hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
                            <i data-lucide="send" class="w-5 h-5"></i>
                        </button>
                    </form>
                </div>
            <?php else: ?>
                <div class="flex-1 flex flex-col items-center justify-center text-center p-12 space-y-4">
